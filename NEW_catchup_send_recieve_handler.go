...
func HandleCatchupSend(pgDataDirectory string, destination string) {
	...
	tracelog.ErrorLogger.FatalOnError(err)
	// Добавление оценки оставшегося времени
   	totalFiles := len(fileList)
   	processedFiles := 0
 	 for fileName, fileDescription := range fileList {
      	     processedFiles++
             remainingFiles := totalFiles - processedFiles
             elapsedTime := time.Since(startTime).Seconds()
             if processedFiles > 0 {
                 avgTimePerFile := elapsedTime / float64(processedFiles)
                 estimatedRemainingTime := avgTimePerFile * float64(remainingFiles)
                 tracelog.InfoLogger.Printf("Processed %v/%v files. Estimated remaining time: %.2f seconds", processedFiles, totalFiles, estimatedRemainingTime)
        }
...
func HandleCatchupReceive(pgDataDirectory string, port int) {
	...
	for {
		...
		// Для оценки общего количества команд можно использовать информацию из команды
		// Например, если CatchupCommandDto содержит поле с общим количеством команд
		if cmd.TotalCommands > 0 {
			totalCommands = cmd.TotalCommands
		}

		// Оценка оставшегося времени
		elapsedTime := time.Since(startTime)
		if processedCommands > 0 && totalCommands > 0 {
			estimatedTotalTime := elapsedTime * time.Duration(totalCommands) / time.Duration(processedCommands)
			remainingTime := estimatedTotalTime - elapsedTime
			tracelog.InfoLogger.Printf("Estimated remaining time: %v", remainingTime)
		}
	}
